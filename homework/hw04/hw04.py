def shuffle(s):
    ...
    s = list(s)
    total_length = len(s)
    remaining_tackled_times = total_length /2 
    tackled_times = 0
    def tackled_sequence(s, remaining_tackled_times, tackled_times):
        if remaining_tackled_times == 0:
            return []
        else:

            return [s[tackled_times]] + [s[tackled_times + int(len(s) / 2)]] + tackled_sequence(s, remaining_tackled_times - 1, tackled_times + 1)


    return tackled_sequence(s, remaining_tackled_times, tackled_times)
...
def deep_map(f, s):
    ...
    """
    "*** YOUR CODE HERE ***"
    for element_number in range(int(len(s))):
        if type(s[element_number]) != list:
            s[element_number] = f(s[element_number])
        else:
            deep_map(f, s[element_number])
...
def balanced(m):
    ...
    is_balanced = left_torque == right_torque
...
def max_path_sum(t):
    ...
    for element in branches(t):
        current_sum = label(t) + max_path_sum(element)
